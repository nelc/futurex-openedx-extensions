...
from futurex_openedx_extensions.helpers.converters import error_details_to_dictionary, dict_to_hash
...
class ThemeConfigDraftView(FXViewRoleInfoMixin, APIView):
    ...
    def put(self, request: Any, tenant_id: int) -> Response:  # pylint: disable=no-self-use
        ...
        try:
            ...
            if new_value is not None and not isinstance(new_value, cs.KEY_TYPE_MAP.get(key_access_info.key_type)):
                raise FXCodedException(
                    code=FXExceptionCodes.INVALID_INPUT,
                    message=f'New value type must be {cs.KEY_TYPE_MAP.get(key_access_info.key_type)} value.'
                )

            if current_value is not None and not isinstance(current_value, cs.KEY_TYPE_MAP.get(key_access_info.key_type)):
                raise FXCodedException(
                    code=FXExceptionCodes.INVALID_INPUT, message=f'Current value type must be {cs.KEY_TYPE_MAP.get(key_access_info.key_type)}.'
                )
