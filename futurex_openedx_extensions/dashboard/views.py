...
class AggregatedCountsView(TotalCountsView):
    ...
    def _load_query_params(self, request: Any) -> None:
        ...
        self.favours_backward = request.query_params.get('favours_backward', '1') == '1'
        ...
        try:
            self.date_from = timezone.datetime.strptime(date_from, '%Y-%m-%d').date() if date_from else None
            self.date_to = timezone.datetime.strptime(date_to, '%Y-%m-%d').date() if date_to else None
    ...
    def _get_enrollments_count_data(  # type: ignore
        ...
        collector_result, calculated_from, calculated_to = get_enrollments_count_aggregated(
            ...
            favours_backward=self.favours_backward,
    ...
    def _construct_result(self) -> dict:
        ...
        for tenant_id in self.tenant_ids:
            ...
            for stat in self.stats:
                ...
                tenant_data[key] = data[0]
                count = sum(item['value'] for item in data[0])
