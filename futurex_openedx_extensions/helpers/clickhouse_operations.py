...
def escape_param(param_name: str, param_value: Any) -> str:
    """
    ...
    if isinstance(param_value, (list, tuple)):
        return '[' + ', '.join(escape_param(param_name, list_item) for list_item in param_value) + ']'

    raise ClickhouseQueryParamsError(f'Unsupported parameter type: ({param_name}: {type(param_value)})')
