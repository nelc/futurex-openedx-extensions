...
def invalidate_tenant_readable_lms_configs(tenant_id: int) -> None:
    ...
    :type tenant_id: int
    """
    cache.delete(cache_name_tenant_readable_lms_configs(tenant_id))
...
def get_tenant_config(tenant_id: int, keys: List[str], published_only: bool = True) -> Dict[str, Any]:
    ...
    if published_only:
        ...
        config_paths = [
            access_control_value['path'] for access_control_key, access_control_value in config_access_control.items()
            if access_control_key in keys
        ]
    ...
    for key in set(keys):
        key = key.strip()
        try:
            key_config = config_access_control[key]
