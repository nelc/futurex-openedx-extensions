...
class TestAggregatedCountsView(BaseTestViewMixin):
    ...
    def setUp(self):
        """Setup"""
        super().setUp()
        self.view = viewz.AggregatedCountsView()
    ...
    @patch('futurex_openedx_extensions.dashboard.viewz.AggregatedCountsView._construct_result')
    @ddt.data(
    ...
    @patch('futurex_openedx_extensions.dashboard.viewz.AggregatedCountsView.get_data_with_missing_periods')
    @ddt.data(True, False)
...
class TestLearnersView(BaseTestViewMixin):
    ...
    def test_no_tenants(self):
        ...
        self.login_user(self.staff_user)
        with patch('futurex_openedx_extensions.dashboard.viewz.get_learners_queryset') as mock_queryset:
    ...
    @patch('futurex_openedx_extensions.dashboard.viewz.get_learners_queryset')
...
class TestLearnerInfoView(
    ...
    def test_success(self):
        ...
        self.login_user(self.staff_user)
        with patch('futurex_openedx_extensions.dashboard.viewz.get_learner_info_queryset') as mock_get_info:
    ...
    @patch('futurex_openedx_extensions.dashboard.viewz.serializers.LearnerDetailsExtendedSerializer')
...
class TestLearnerCoursesDetailsView(
    ...
    def test_success(self):
        ...
        self.login_user(self.staff_user)
        with patch('futurex_openedx_extensions.dashboard.viewz.get_learner_courses_info_queryset') as mock_get_info:
    ...
    @patch('futurex_openedx_extensions.dashboard.viewz.serializers.LearnerCoursesDetailsSerializer')
    def test_request_in_context(self, mock_serializer):
...
class TestAccessibleTenantsInfoView(BaseTestViewMixin):
    ...
    @patch('futurex_openedx_extensions.dashboard.viewz.get_user_by_username_or_email')
...
class TestGlobalRatingView(BaseTestViewMixin):
    ...
    def test_success(self):
        ...
        with patch('futurex_openedx_extensions.dashboard.viewz.get_courses_ratings') as mocked_calc:
...
class TestClickhouseQueryView(MockPatcherMixin, BaseTestViewMixin):
    ...
    patching_config = {
        'get_query_record': ('futurex_openedx_extensions.dashboard.viewz.ClickhouseQuery.get_query_record', {
            'side_effect': MockClickhouseQuery.get_query_record,
        }),
        'parse_query': ('futurex_openedx_extensions.dashboard.viewz.ClickhouseQuery.fix_param_types', {
        ...
        'get_usernames_with_access_roles': (
            'futurex_openedx_extensions.dashboard.viewz.get_usernames_with_access_roles',
            {'return_value': []}
        ),
    }
    ...
    def test_get_page_url_with_page(self, param_string, new_page_no, expected_result):
        ...
        self.assertEqual(viewz.ClickhouseQueryView.get_page_url_with_page(url, new_page_no), expected_result)
    ...
    def test_pop_out_page_params(self, params, paginated, expected_result):
        ...
        self.assertEqual(viewz.ClickhouseQueryView.pop_out_page_params(params, paginated), expected_result)
...
class FileUploadView(BaseTestViewMixin):
    ...
    @patch('futurex_openedx_extensions.dashboard.viewz.uuid.uuid4')
    @patch('futurex_openedx_extensions.dashboard.viewz.get_storage_dir')
...
class TestCategoriesView(MockPatcherMixin, BaseTestViewMixin):
    ...
    patching_config = {
        'one_tenant': (
            'futurex_openedx_extensions.dashboard.viewz.FXViewRoleInfoMixin.verify_one_tenant_id_provided', {
                'return_value': 1,
            }
        ),
    }
...
class TestLearnerUnenrollView(BaseTestViewMixin):
    ...
    def test_view_has_correct_permissions(self):
        ...
        view = viewz.LearnerUnenrollView()
    ...
    def test_unenroll_permission_denied_for_course_org(self):
        ...
        original_post = viewz.LearnerUnenrollView.post
        ...
        with patch.object(viewz.LearnerUnenrollView, 'post', patched_post):
            data = {
                'user_key': self.test_user.username,
    ...
    @patch('futurex_openedx_extensions.dashboard.serializers.get_user_by_key')
    @patch('futurex_openedx_extensions.dashboard.viewz.get_course_search_queryset')
