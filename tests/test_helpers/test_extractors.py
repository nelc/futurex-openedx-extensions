...
@pytest.mark.parametrize(
    'period, date_point, target_forward, max_chunks, expected',
    [
        ('day', d_t('2024-01-01'), True, 5, d_t('2024-01-05')),
        ('month', d_t('2024-01-01'), True, 3, d_t('2024-03-31')),
        ('month', d_t('2024-01-31'), True, 3, d_t('2024-03-31')),
        ('quarter', d_t('2024-01-01'), True, 1, d_t('2024-03-31')),
        ('quarter', d_t('2024-01-20'), True, 2, d_t('2024-06-30')),
        ('quarter', d_t('2024-01-01'), False, 1, d_t('2024-01-01')),
        ('quarter', d_t('2024-01-01'), False, 2, d_t('2023-10-01')),
        ('year', d_t('2024-01-01'), True, 1, d_t('2024-12-31')),
        ('year', d_t('2024-01-20'), False, 1, d_t('2024-01-01')),
        ('year', d_t('2024-01-20'), False, 2, d_t('2023-01-01')),
        ('day', d_t('2024-01-01'), True, -1, None),
    ],
)
def test_get_valid_date_duration(period, date_point, target_forward, max_chunks, expected):
    """Verify that get_valid_date_duration returns the expected date."""
    assert get_valid_date_duration(period, date_point, target_forward, max_chunks) == expected
...
def test_get_valid_date_duration_default_chunks():
    ...
    assert get_valid_date_duration('day', d_t('2024-01-01'), True, 0) == d_t('2024-01-10')
    assert get_valid_date_duration('month', d_t('2024-01-20'), False, 0) == d_t('2023-12-01')
...
def test_get_max_valid_date_to(mock_get_valid_date_duration):
    ...
    mock_get_valid_date_duration.return_value = d_t('2024-12-26')
    ...
    mock_get_valid_date_duration.assert_called_once_with(
        period='a_period', date_point='a_date_from', target_forward=True, max_chunks='a_chunk_value'
    )
...
def test_get_min_valid_date_from(mock_get_valid_date_duration):
    ...
    mock_get_valid_date_duration.assert_called_once_with(
        period='a_period', date_point='a_date_to', target_forward=False, max_chunks='a_chunk_value'
    )
...
def test_get_valid_duration_favour_backward_no_effect(date_from, date_to, expected_dates):
    """
    Verify that get_valid_duration returns the same value regardless of the favours_backward parameter incase one of
    ...
    for favours_backward in [True, False]:
        assert get_valid_duration(period, date_from, date_to, favours_backward, max_chunks) == expected_result


@pytest.mark.parametrize(
    'date_from, date_to, favours_backward, expected_dates',
    [
...
def test_get_valid_duration_favour_backward(date_from, date_to, favours_backward, expected_dates):
    """
    Verify that get_valid_duration returns the correct value according to favours_backward parameter incase date_from
...
def test_get_valid_duration_negative_max_chunks(date_from, date_to, expected_dates):
    ...
    max_chunks = -1
    for favours_backward in [True, False]:
        assert get_valid_duration(period, date_from, date_to, favours_backward, max_chunks) == (
            datetime.combine(expected_dates[0], datetime.min.time()) if expected_dates[0] else None,
            datetime.combine(expected_dates[1], datetime.max.time()) if expected_dates[1] else None,
        )
