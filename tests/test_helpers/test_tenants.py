...
def test_generate_tenant_config_success(mock_get):
    ...
    mock_default_config.lms_configs = {
        ...
        'EOX_THEMING_DEFAULT_THEME_NAME': '{{platform_name}}-edx-theme',
        'LMS_BASE': '{{platform_name}}.local.overhang.io',
        'SITE_NAME': 'http://{{platform_name}}.local.overhang.io:8000/',
        'course_org_filter': ['{{platform_name}}_org'],
    }
    mock_get.return_value = mock_default_config
    result = tenants.generate_tenant_config('testplatform')
    assert result['EOX_THEMING_DEFAULT_THEME_NAME'] == 'testplatform-edx-theme'
    assert result['LMS_BASE'] == 'testplatform.local.overhang.io'
...
def test_generate_tenant_config_tenant_not_found(mock_get):
    ...
    with pytest.raises(FXCodedException) as excinfo:
        tenants.generate_tenant_config('testplatform')
    assert excinfo.value.code == FXExceptionCodes.TENANT_NOT_FOUND.value
    assert str(excinfo.value) == 'Default TenantConfig not found.'
...
def test_create_new_tenant_config_success(mock_route_create, mock_tenant_create, mock_generate_config):
    ...
    result = tenants.create_new_tenant_config('testplatform')
    mock_generate_config.assert_called_once_with('testplatform')
