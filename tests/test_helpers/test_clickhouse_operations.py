...
def test_escape_param(param_value, expected_result):
    """Verify that escape_param works as expected."""
    result = ch.escape_param('param', param_value)
    assert result == expected_result


def test_escape_param_unsupported_type():
    """Verify that escape_param raises ClickhouseQueryParamsError when unsupported type."""
    with pytest.raises(ch.ClickhouseQueryParamsError) as exc_info:
        ch.escape_param('param', object())
    assert exc_info.value.args[0] == 'Unsupported parameter type: (param: <class \'object\'>)'
