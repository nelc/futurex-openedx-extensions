...
@override_settings(FX_COURSE_CATEGORY_MAX_COUNT=5)
...
@override_settings(FX_COURSE_CATEGORY_MAX_COUNT=1)
def test_get_new_category_name_bad_settings(
    patched_config_loader,
):  # pylint: disable=redefined-outer-name, unused-argument
    """Verify get_new_category_name will fail when FX_COURSE_CATEGORY_MAX_COUNT is less than 2."""
    course_cat = CourseCategories(1)
    course_cat.categories = {}
    assert len(course_cat.categories) == 0, 'bad test data: categories should be empty'
